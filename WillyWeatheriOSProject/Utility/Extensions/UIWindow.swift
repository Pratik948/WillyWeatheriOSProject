...
extension UIWindow {
    public func swapRootViewControllerWithAnimation(newViewController:UIViewController, animationType:SwapRootVCAnimationType, completion: (() -> ())? = nil) {
        let direction:TransitionOptions.Direction = animationType == .push ? .toRight : animationType == .pop ? .toLeft : animationType == .present ? .toTop : animationType == .dismiss ? .toBottom : .fade
        var option = UIWindow.TransitionOptions.init(direction: direction, style: .easeOut)
        option.background = TransitionOptions.Background.snapshot
        self.setRootViewController(newViewController, options: option) {
            completion?()
        }
    }
}
...
public extension UIWindow {
    ...
    func setRootViewController(_ controller: UIViewController, options: TransitionOptions = TransitionOptions(), completion:(()->())?=nil) {
